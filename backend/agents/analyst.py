...
class AnalystAgent:
    ...
    def _build_prompt(self, reader_results: list[ReaderResult]) -> str:
        ...
        return (
            "You are a rigorous research analyst preparing material for an academic-style report.\n"
            "Below are key points extracted from multiple sources.\n\n"
            "Your tasks:\n"
            "1. Write 4-6 numbered, citable claims that synthesize the most important factual information across the sources.\n"
            "   - Format each claim as: (1) Claim text [Source N].  where N is the source number the claim comes from.\n"
            "   - Each claim must be specific, factual, and non-redundant.\n"
            "2. List any genuine contradictions or disagreements between sources.\n"
            "   - Format each contradiction as: 1. Source X claims ..., while Source Y states ...\n"
            "   - Only list real contradictions supported by the text. If none exist, write 'None found.'\n\n"
            "Use this EXACT format in your response (both headers are required):\n"
            "SUMMARY:\n"
            "(1) <first citable claim> [Source N].\n"
            "(2) <second citable claim> [Source N].\n"
            "...\n\n"
            "CONTRADICTIONS:\n"
            "1. <contradiction naming which sources disagree and how>\n"
